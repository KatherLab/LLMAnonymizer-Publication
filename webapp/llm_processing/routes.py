...
def main():
    ...
    if form.validate_on_submit():
        ...
        # check if tokenizer_config.json and tokenizer.json in current_app.config['TOKENIZER_PATH']
        if not os.path.exists(os.path.join(current_app.config['TOKENIZER_PATH'], 'tokenizer_config.json')) or not os.path.exists(os.path.join(current_app.config['TOKENIZER_PATH'], 'tokenizer.json')):
            flash("Tokenizer json files not found. Did you specify --tokenizer_path correctly? Cannot check if input fits, skip.", "warning")
        else:
